...
class KITTI(torch.utils.data.Dataset):
    def __init__(self, cfgs):
        ...
        if self.split == 'training200':
            self.indices = np.arange(int(np.floor(294/2))) #TODO: Change value depending on the dataset
        elif self.split == 'training160':
            self.indices = [i for i in range(160) if i % 5 != 0]
        elif self.split == 'training40':
            self.indices = [i for i in range(40) if i % 5 == 0]
...
class KITTITest(torch.utils.data.Dataset):
    def __init__(self, cfgs, qty):
        ...
        self.split = cfgs.split
        self.cfgs = cfgs
        self.indices = np.arange(int(np.floor(qty/2)))
    ...
    def __getitem__(self, i):
        ...
        proj_mat = load_calib(os.path.join(self.root_dir, 'calib_cam_to_cam', '%06d_10.txt' % index))
        f, cx, cy = proj_mat[0, 0], proj_mat[0, 2], proj_mat[1, 2]
        ...
        #print(f' ---- The PC1 shape is {pc1.shape} ---- ')
        #print(f' ---- The PC2 shape is {pc2.shape} ---- ')
