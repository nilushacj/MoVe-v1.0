    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                logging.info(f'DET SHAPE: {det.shape}')
                # ---- Sort the bounding boxes with the largest areas if there are at least two ----
                if len(det) > 1:
                    ...
                    logging.info(f'BEFORE 2 SORTED AREA IDX: {idx_area_sorted}')
                    orig_idx_area_sorted = idx_area_sorted.copy()
                    # -- Check for possible cyclists --
                    if len(det) > 2:
                        # -- Get row indexes of top two --
                        idx_area_sorted = idx_area_sorted[:2]
                        # -- Remove all rows apart from the top 2 (based on area) --
                        pass_idx = torch.tensor(idx_area_sorted, device=device)
                    ...
                    logging.info(f'REMAINING CLASSES: {remaining_classes}')

                    # ---- Get areas again (since idxs changed to just 2) ----
                    bi_dim_2D, bi_coords_2D, bi_det_areas = append_dim_coord(det)
                    # ---- Sort the two area indexes in descending order ----
                    bi_idx_area_sorted = sorted(range(len(bi_dim_2D)), key=lambda i: bi_det_areas[i], reverse=True)

                    possible_cyclist_flag = False
                    highest_class = det[orig_idx_area_sorted[0], 5].cpu().numpy()
                    highest_class = highest_class.astype(int)
                    # ---- Check if largest area is a person, cycle or bike ----
                    if highest_class in [0,1,3]:
                        # ---- Check if its a person ----
                        if highest_class==0:
                            # ---- Check if any remaining elements is a bike or bicycle ----
                            for i in range(1,len(orig_idx_area_sorted)):
                                current_class = det[orig_idx_area_sorted[i], 5].cpu().numpy()
                                current_class = current_class.astype(int)
                                if current_class in [1,3]:
                                    possible_cyclist_flag = True
                                    break
                        # ---- Check if its a bike/cycle ----
                        elif highest_class in [1,3]:
                            # ---- Check if any remaining elements is a person ----
                            for i in range(1,len(orig_idx_area_sorted)):
                                current_class = det[orig_idx_area_sorted[i], 5].cpu().numpy()
                                current_class = current_class.astype(int)
                                if current_class==0:
                    ...
                    # ---- If the 2 bounding boxes remaining are NOT a combination of bicycle/bike and a person ----
                        ...
                        # -- Overwrite original output --
                        det = det_not_cyc.clone()
                logging.info(f'FINAL YOLO DET SHAPE: {det.shape}')
                ...
                #print(f'BOUNDING BOXESS RESCALED:{det[:, :4]}')
