    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # ---- If not cyclist ----
                if cyclist_flag=='False':
                    logging.info('  KITTI says not a cyclist')
                    if len(det) == 1:
                        temp = det[:, 5]
                        temp = temp.cpu().numpy()
                        temp = temp.astype(int)[0]
                        logging.info(f'    Class of single instance: {temp}')
                    # ---- Get the bounding box with the highest areas if there are at least two ----
                    else:
                        ...
                        logging.info(f'    Classes of multiple instances: {idx_area_sorted}')
                        ...
                        # -- Get row indexes of top class --
                        rmd_idx_area_sorted = idx_area_sorted[:1]
                        ...
                        det = det_new.clone() # -- new det made with new index --
                    
                # ---- If cyclist (main script should check if there are 2 road users) ----
                else:
                    logging.info('  KITTI says its a cyclist')
                    # ---- Only one road user is found ----
                    if len(det) == 1:
                        # ---- Do nothing (main script takes care) ----
                        temp = det[:, 5]
                        temp = temp.cpu().numpy()
                        temp = temp.astype(int)[0]
                        logging.info(f'    Class of single instance: {temp}')
                        ...
                        # ---- Check if there is at least one bicycle/bike ----
                        p_exists = 0 in all_classes
                        c_exists = 1 in all_classes or 3 in all_classes 
                        # ---- If both flags are true ----
                        if p_exists and c_exists: 
                            logging.info('    Person and bicycle exists')
                            # -- Sort area indexes in descending order --
                            dim_2D, coords_2D, det_areas = append_dim_coord(det)
                            idx_area_sorted = sorted(range(len(dim_2D)), key=lambda i: det_areas[i], reverse=True)
                            logging.info(f'    Classes of multiple instances: {idx_area_sorted}')
                            ...
                            c_idx = -1
                            c_area = -1     
                            # -- Get index of person and bicycle with highest area --
                            for pc_desc_count in range(len(idx_area_sorted)):
                                temp_class = det[idx_area_sorted[pc_desc_count], 5].cpu().item()
                                temp_class = int(temp_class)
                                temp_area  = det_areas[idx_area_sorted[pc_desc_count]]
                                temp_conf  = det[idx_area_sorted[pc_desc_count], 4].cpu().item()
                                temp_conf  = float(temp_conf) #.astype(float)
                                logging.info('      %s (area: %s pixels, confidence: %.2f)'%(temp_class, int(temp_area), temp_conf))
      
                                # -- Person case --
                                if temp_class==0:
                                    temp_p_area  = det_areas[idx_area_sorted[pc_desc_count]]
                                    if temp_p_area>p_area:
                                        p_area = temp_p_area
                                        p_idx = idx_area_sorted[pc_desc_count]
                                # -- Bicycle case --
                                if temp_class==1 or temp_class==3:
                                    temp_c_area  = det_areas[idx_area_sorted[pc_desc_count]]
                                    if temp_c_area>c_area:
                                        c_area = temp_c_area
                                        c_idx = idx_area_sorted[pc_desc_count]
                            # -- Remove all indexes except for the person and bicycle/bike --
                            logging.info(f'    Bicyle index: {c_idx}, Person index: {p_idx}')
                            pass_idx = torch.tensor([p_idx,c_idx], device=device)
                            filter_mask = torch.zeros(det.shape[0], dtype=torch.bool, device=device)
                            filter_mask[pass_idx] = True
                            det_new = det[filter_mask]
...
def parse_opt():
    ...
    parser.add_argument('--cyclist_flag', type=str, help='cyclist or not')
