...
def plot_masks(img, masks, colors, alpha=0.5):
    ...
    img_new = img_gpu.clone()#np.zeros_like(img_gpu, dtype=np.uint8)
    img_new.zero_()
    num_masks = len(masks)
    if num_masks == 0:
    ...
    img_new = img_new.flip(dims=[0])  # filp channel for opencv
    img_new = img_new.permute(1, 2, 0).contiguous()
    # [h, w, 3]
    img_new = img_new * inv_alph_masks.prod(dim=0) + masks_color_summand
    return (img_new * 255).byte().cpu().numpy()
